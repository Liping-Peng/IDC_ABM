...
| `LP_imprinting` | Boolean flag for simulating historical immune imprinting effects. |
| `LP_preHAI` | Boolean flag for pre-assigning baseline Hemagglutination Inhibition (HAI) titers at initialization. |
...
| `LP_mismatch_age` | Boolean flag to incorporate age-specific incidence data for model calibration. |
| `LP_mismatch_absent` | Boolean flag to incorporate school absenteeism data for model calibration. |
...
| `LP_corr_layer` | Contact layer(s) where trait correlations are enforced (e.g., `'all'` for universal, `'h'` for household only). |
...
* **Network Layer Weights (`beta_layer`):** Updated to `dict(h=2.0, s=0.5, w=0.5, c=0.2)` to reflect empirical influenza contact patterns.
* **Layer Contact Counts (`contacts`):** Updated to `dict(h=2.0, s=18, w=18, c=20)` to align with the target population structure.
* **Infectiousness Variance (`beta_dist`):** Altered dispersion (`par2=1.0`) to establish the required baseline variance in individual-level peak viral loads.
* **Quarantine Duration (`quar_period`):** Shortened to `7` days to match standard influenza isolation protocols.
* **Immunity Kinetics:** 
    * Switched to an additive NAb boost multiplier (`nab_boost = 4.0`).
    * Transitioned to a linear antibody decay model (`decay_rate1 = 0.14`).
    * Removed breakthrough transmission reductions (`trans_redux = 1.0`).

#### 1.3 Complete Prognoses Overhaul

Replaced default relative COVID-19 disease progression with absolute influenza-specific parameters:
* **Age Stratification:** Reduced age groups from 10 to 5 broad bins: `[0, 12, 25, 45, 65]`.
* **Clinical Outcomes:** Flattened all severe, critical, and fatal trajectories to zero (`severe_probs = crit_probs = death_probs = 0.0`) to strictly isolate structural transmission dynamics.

#### 1.4 Custom Vaccine & Variant Profiles

Added pathogen-specific definitions for evaluation:
* **`lpstrain`**: Custom theoretical viral strain baseline.
* **`lp_flu_30`**: Theoretical vaccine profile providing fixed 30% efficacy (simulating a vaccine-mismatch season).
* **`lp_flu_70`**: Theoretical vaccine profile providing fixed 70% efficacy (simulating a vaccine-match season).

#### 1.5 New Helper Functions & Data Dependencies

Added three utility functions to load and map empirical immunological data onto the synthetic population:
* **`get_imprt_data()`**: Loads immune imprinting probabilities from `LP_input data/imprinting_prob/sample_year_2019.csv`.
* **`get_preHAI_data()`**: Loads baseline HAI titer distributions from `LP_input data/hai_protection/HAI_distribution.csv`.
...
The `population.py` module was modified to allow the direct injection of pathogen-specific, age-stratified natural history parameters during initialization. 
...
The core `Sim` class was modified to implement the **infectiousness-dependence mechanism (IDC)** . These updates enable the simulation of correlated biological traits across transmission pairs while maintaining a strictly controlled baseline population.

#### 5.2 Key Epidemiological Updates
