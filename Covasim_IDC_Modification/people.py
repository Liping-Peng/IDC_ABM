...
#%% Imports
# LP_mod: Imported pandas, random, math for advanced probabilistic calculations and matrix operations.
import numpy as np
import pandas as pd
import sciris as sc
...
class People(cvb.BasePeople):
    ...
    **Examples**::
    ...
    def set_prognoses(self):
        ...
        # print ("---***********LP_people.py:pars['prognoses']['symp_probs']:",pars['prognoses']['symp_probs'])
    ...
    def infect(self, inds, hosp_max=None, icu_max=None, source=None, layer=None, variant=0, LP_symDur_index_prop=0,LP_infDur_index_prop=0, LP_corr_layer='all'):
        '''
        ...
        LP_mod: Expanded the parameter list of the infect method to include LP_symDur_index_prop, LP_infDur_index_prop, and LP_corr_layer.
        These parameters are introduced to mathematically model the correlation of symptom and infectious durations between index cases and secondary cases across specific network layers.
        ...
        Args:
            ...
            LP_symDur_index_prop (float): A float weight (0 to 1) determining the degree to which the infector's symptom duration influences the infectee's symptom duration.
            LP_infDur_index_prop (float): A float weight (0 to 1) dictating the correlation strength between the infector's and infectee's infectious durations.
            LP_corr_layer       (str): A string (e.g., 'h', 'all') specifying the contact network layer where this trait correlation is enforced.
        ...
        for i, target in enumerate(inds):
            # entry = dict(source=source[i] if source is not None else None, target=target, date=self.t, layer=layer, variant=variant_label)   # LP_mod: Commented out default code
            # LP_mod: Extended infection log with age information
        ...
        # LP_mod: Commented out default duration logic
        ...
        # LP_mod: Replaced the default static symptom probability with a dynamically calculated probability using a logit transformation.
        # This incorporates the individual's infectiousness (representing peak viral load) via the LP_coef_InfSym coefficient, theorizing that higher viral loads statistically correlate with a greater likelihood of symptom development
        # step 1: Compute logit_pre_symp_prob, the log-odds of the baseline symptom probability.
        pre_symp_prob = infect_pars['rel_symp_prob'] * self.symp_prob[inds] * (1 - self.symp_imm[variant, inds])
        logit_pre_symp_prob = np.log(pre_symp_prob / (1 - pre_symp_prob))

        # step2: Calculate logit_InfSym, the log-odds offset driven by peak viral load (rel_trans), scaled by the coefficient LP_coef_InfSym.
        LP_coef_InfSym = self.pars['LP_coef_InfSym'] # 为传染力影响症状概率的系数
        logit_InfSym = LP_coef_InfSym * (np.log(self.rel_trans[inds]) - 1)

        # step 3: Apply the inverse logit (sigmoid) function to the summed logits to yield the adjusted probability array (symp_probs).
        logit_final = logit_pre_symp_prob + logit_InfSym
        symp_probs = 1 / (1 + np.exp(-logit_final))
        ...
        # LP_mod: default formula, which assumed the infectious duration is independent from the index.
        # # default: CASE 1: Asymptomatic: may infect others, but have no symptoms and do not die
        ...
        # # default: CASE 2: Symptomatic: can either be mild, severe, or critical
        ...
        # # default: CASE 2.1: Mild symptoms, no hospitalization required and no probability of death
        ...
        # LP_mod: Add correlation of (1) infectious duration  and (2) symptom duration between index and secondary cases
        # CASE 1: Asymptomatic: may infect others, but have no symptoms and do not die
        # LP_mod: Add correlation of infectious duration (for asymptomatic cases)
        dur_asym2rec = cvu.sample(**durpars['asym2rec'], size=len(asymp_inds))
        if (source is not None) and (len(asymp_inds)>0) :
            source_asym_inds = [source[i] for i, value in enumerate(inds) if value in asymp_inds]
            source_infDur = self.date_recovered[source_asym_inds] - self.date_infectious[source_asym_inds]   # correspond to the following code: self.date_recovered[asymp_inds] = self.date_infectious[asymp_inds] + dur_asym2rec
            if LP_corr_layer == 'all' or LP_corr_layer == layer:   # control trans_pattern apply in: 1) all setting; 2) only in the layer we set ('h')
                dur_asym2rec = (1- LP_infDur_index_prop) * dur_asym2rec + LP_infDur_index_prop * source_infDur   ## random + index_infDur
        self.date_recovered[asymp_inds] = self.date_infectious[asymp_inds] + dur_asym2rec  # Date they recover
        self.dur_disease[asymp_inds] = self.dur_exp2inf[asymp_inds] + dur_asym2rec  # Store how long this person had COVID-19


        # CASE 2: Symptomatic: can either be mild, severe, or critical
        # LP_mod: Add correlation of infectious duration (for symptomatic cases). Here is Step1, because infectious duration of symp cases is divided into inf2sym and sym2rec, therefore adjust them separately.
        n_symp_inds = len(symp_inds)
        temp_dur_inf2sym = cvu.sample(**durpars['inf2sym'], size=n_symp_inds)
        if (source is not None) and (len(symp_inds)>0) :
            source_sym_inds = [source[i] for i, value in enumerate(inds) if value in symp_inds]
            source_inf2sym = self.date_symptomatic[source_sym_inds] - self.date_infectious[source_sym_inds]   # because dur_disease of symp cases is divided into inf2sym and sym2rec, so cannot use dur_disease as in asymptomatic cases
            if LP_corr_layer == 'all' or LP_corr_layer == layer:   # control trans_pattern apply in: 1) all setting; 2) only in the layer we set ('h')
                temp_dur_inf2sym = np.where(
                    np.isnan(source_inf2sym),  # Condition: Check for NaN in source_symDur  ## 如果source为asymp（即asymp传给symp）
                    temp_dur_inf2sym,  # If True (NaN), use temp_dur_inf2sym
                    (1- LP_infDur_index_prop) * temp_dur_inf2sym + LP_infDur_index_prop * source_inf2sym  # If False (not NaN), use the weighted sum  ## random + index_infDur
                )

        self.dur_inf2sym[symp_inds] = temp_dur_inf2sym # Store how long this person took to develop symptoms
        self.date_symptomatic[symp_inds] = self.date_infectious[symp_inds] + self.dur_inf2sym[symp_inds] # Date they become symptomatic
        sev_probs = infect_pars['rel_severe_prob'] * self.severe_prob[symp_inds]*(1-self.sev_imm[variant, symp_inds]) # Probability of these people being severe
        is_sev = cvu.binomial_arr(sev_probs) # See if they're a severe or mild case
        sev_inds = symp_inds[is_sev]
        ...
        # LP_mod: (1) add correlation of infectious duration (Here is step 2); (2) add correlation of symptom duration (only for symptomatic cases)
        temp_dur_mild2rec = cvu.sample(**durpars['mild2rec'], size=len(mild_inds))
        if (source is not None) and (len(symp_inds)>0) :
            source_mild_inds = [source[i] for i, value in enumerate(inds) if value in mild_inds]
            source_symDur = self.date_recovered[source_mild_inds] - self.date_symptomatic[source_mild_inds]
            LP_index_prop =  max(LP_symDur_index_prop, LP_infDur_index_prop)
            if LP_corr_layer == 'all' or LP_corr_layer == layer:  # control trans_pattern apply in: 1) all setting; 2) only in the layer we set ('h')
                dur_mild2rec = np.where(
                    np.isnan(source_symDur),  # Condition: Check for NaN in source_symDur
                    temp_dur_mild2rec,  # If True (NaN), use temp_dur_mild2rec
                    (1 - LP_index_prop) * temp_dur_mild2rec + LP_index_prop * source_symDur  # If False (not NaN), use the weighted sum
                )
            else:
                dur_mild2rec = temp_dur_mild2rec
        else:
            dur_mild2rec = temp_dur_mild2rec
        self.date_recovered[mild_inds] = self.date_symptomatic[mild_inds] + dur_mild2rec  # Date they recover
        self.dur_disease[mild_inds] = self.dur_exp2inf[mild_inds] + self.dur_inf2sym[mild_inds] + dur_mild2rec  # Store how long this person had COVID-19
