...
'''
# LP_mod: Imported mathematical and plotting libraries to support log-transformed infectiousness calculations and advanced data logging.
import random
...
class Sim(cvb.BaseSim):
    ...
    **Examples**::
    ...
    def init_people(self, popdict=None, init_infections=False, reset=False, verbose=None, **kwargs):
        ...
        # LP_mod: Use custom natural history parameters and initialized metadata attributes for epidemiological tracking.
        LP_symp_probs = self.pars['LP_symp_probs']  # Age-specific probabilities of symptom manifestation.
        LP_trans_ORs = self.pars['LP_trans_ORs']    # Age-specific odds ratios for transmissibility.
        LP_sus_ORs = self.pars['LP_sus_ORs']        # Age-specific odds ratios for susceptibility.
        self.people = cvpop.make_people(self, reset=reset, verbose=verbose, LP_sus_ORs = LP_sus_ORs, LP_symp_probs=LP_symp_probs, LP_trans_ORs=LP_trans_ORs, **kwargs)
        self.people.initialize(sim_pars=self.pars) # Fully initialize the people
        self.reset_layer_pars(force=False) # Ensure that layer keys match the loaded population


        # LP_mod: Initialized custom attributes on the People object to track immunological states (e.g., imprinting) and granular transmission network data (LP_trans_log).
        ...
        self.people.LP_iso_log = []
        self.people.LP_trans_log = []
    ...
    def init_interventions(self):
        ...
        for i,intervention in enumerate(self['interventions']):
            # print ("LP_note:sim:intervention: ", intervention)
    ...
    def step(self):
        ...
        prel_trans = people.rel_trans    ## LP_note(20250915):既往设定中people.rel_trans初始化后不会改变，但现在修改为会动态改变，详见下面self.people.rel_trans[target_inds] =
        ...
        for variant in range(nv):
            ...
            for lkey, layer in contacts.items():
                ...
                rel_trans, rel_sus = cvu.compute_trans_sus(prel_trans, prel_sus, inf_variant, sus, beta_layer, viral_load, symp, iso, quar, asymp_factor, iso_factor, quar_factor, sus_imm)  ## LP_note (20250915)：不在此处修改index与secondary的infectiousness的相关性
                ...
                for p1,p2 in pairs:
                    ...
                    # LP_mod: Expanded the infect method call to pass temporal correlation parameters.
                    people.infect(inds=target_inds, hosp_max=hosp_max, icu_max=icu_max, source=source_inds, layer=lkey, variant=variant,
                                  LP_symDur_index_prop = self.pars['LP_symDur_index_prop'],
                                  LP_infDur_index_prop = self.pars['LP_infDur_index_prop'],
                                  LP_corr_layer = self.pars['LP_corr_layer'],)  # Actually infect people


                    # LP_mod: Core process to apply IDC mechanism. Implemented a mean-centered, log-linear correlation model.
                    # This dynamically overrides the infectee's intrinsic infectiousness by inheriting a weighted proportion of the index case's peak infectiousness.
                    if (self.pars['LP_corr_layer'] == 'all' or self.pars['LP_corr_layer'] == lkey) and (self.pars['LP_trans_index_prop'] > 0):
                        ## LP_mod: Convert infectiousness to log space, apply CENTERED relationship, then covert it back to the original scale
                        log_source_trans = np.log(self.people.rel_trans[source_inds])  ## this is the pre-set intrinsic infectiousness
                        log_target_preset_trans = np.log(people.rel_trans[target_inds])  ## this is the pre-set intrinsic infectiousness

                        # step 1. Calculate mu (Replace 'LP_Isd' with your exact dict key for intrinsic SD, e.g., 1.2)
                        infectiousness_Isd = self.pars['beta_dist']['par2']
                        print(infectiousness_Isd)
                        mu = -0.5 * np.log(infectiousness_Isd ** 2 + 1)

                        # step 2. Apply centered formula (subtract mu before multiplying weights, add mu back at the end)
                        log_target_trans = mu + (log_source_trans - mu) * self.pars['LP_trans_index_prop'] + (log_target_preset_trans - mu) * math.sqrt(1 - (self.pars['LP_trans_index_prop']) ** 2)

                        self.people.rel_trans[target_inds] = np.exp(log_target_trans)  # Convert back the result to original (positive) scale using exp()


                    # LP_mod: Logged granular transmission pair metadata—including ages, assigned layers, and dynamic infectiousness values—into a custom dataframe array for network analysis.
                    temp_log = pd.DataFrame({'source':self.people.uid[source_inds],
                                             'target': self.people.uid[target_inds],
                                             'src_age': self.people.age[source_inds],
                                             'trg_age': self.people.age[target_inds],
                                             'src_infectious': self.people.rel_trans[source_inds],
                                             'trg_infectious': self.people.rel_trans[target_inds],
                                             'layer':lkey,
                                             'date':self.t})
                    self.people.LP_trans_log.append(temp_log)
    ...
    def run(self, do_plot=False, until=None, restore_pars=True, reset_seed=True, verbose=None):
        ...
        errormsg = None
        until = self.npts if until is None else self.day(until) ### LP note:self.npts也是关于模型停止运行时间的参数，与until和end_date相似？
        if until > self.npts:
    ...
    def compute_gen_time(self):
        ...
        # LP_mod: Appended median statistical calculations for both true and clinical generation times to the standard summary dictionary output.
        self.results['gen_time'] = {
                'true':         np.mean(intervals1[:pos1]),
                'true_median': np.median(intervals1[:pos1]),
                'true_std':     np.std(intervals1[:pos1]),
                'clinical':     np.mean(intervals2[:pos2]),
                'clinical_median': np.median(intervals2[:pos2]),
