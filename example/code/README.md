...
* `run\_example.py`: **Main execution script.** Parses scenario configurations, initializes the synthetic population, applies interventions, executes the simulation loop, and extracts transmission trees.
* `base\_func.py`: **Analytical module.** Contains post-simulation utility functions to compute individual reproduction numbers, age-stratified attack rates, cross-layer transmission proportions, and generation intervals.
* `base\_data.py`: **Empirical data repository.** Defines disease progression distributions (latent/infectious periods) and baseline demographic arrays.
